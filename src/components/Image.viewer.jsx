...
function Viewer(props) {
  ...
  const imgDimensions = () => {
    ...
    let newScale = 1;
    if (img !== null) {
      const { naturalWidth: currentWidth, naturalHeight: currentHeight } = img;
  ...
  const peopleToSearchData = (data) => {
    ...
    for (let i = 0; i < length; i += 1) {
      const { ref = null } = data[i];
      if (ref !== null) {
        const { _id, label: refLabel = '', person = null } = ref;
        let altName = '';
        const name = refLabel.replace(/\s\s/g, ' ');
        if (person !== null && person.label !== '') {
          const personLabel = person.label.replace(/\s\s/g, ' ');
          if (personLabel !== name) {
            altName = personLabel;
          }
        }
  ...
  }, [loading, visible, dimensionsLoaded]);
  ...
  const centerNode = (_id) => {
    ...
    const node = nodes.find((n) => n.ref._id === _id) || null;
    if (node !== null) {
  ...
  const renderSearchItemHTML = (n) => {
    const { _id = null, name = '', altName = '' } = n;
    if (_id === null) {
      return false;
    }
    const rLabel = [];
    if (name !== '') {
      rLabel.push(<span key="name">{name}</span>);
    }
    if (altName !== '') {
      rLabel.push(<br key="break" />);
      rLabel.push(<small key="alname">[{altName}]</small>);
    }
    return (
      <div
        key={_id}
        data-id={_id}
        onClick={() => centerNode(_id)}
        onKeyDown={() => false}
        role="button"
        tabIndex={0}
  ...
  const tooltipVisibleClass = tooltipVisible ? ' active' : '';
  const tooltipHTML = (
    <div
  ...
  return visible ? (
    <div className="classpiece-viewer">
      <div
