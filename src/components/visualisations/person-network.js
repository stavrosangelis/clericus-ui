...
const drawNodes = (coords) => {
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    ...
    if (newX > 0 && newX < width && newY > 0 && newY < height) {
      ...
      let { radius } = d;
      d.visible = true;
      const labelRows = d.label.split(' ').filter((l) => l !== '');
      ctx.beginPath();
      // circle
...
const drawLines = (coords) => {
  ...
  for (let i = 0; i < links.length; i += 1) {
    ...
    const newX = coordsX + tx * scale;
    const newY = coordsY + ty * scale;
    if (newX > 0 && newX < width && newY > 0 && newY < height) {
      ctx.beginPath();
      ctx.moveTo(sx, sy);
      ctx.lineTo(tx, ty);
...
const zooHandler = d3.zoom().scaleExtent([0.1, 8]).on('zoom', zoomedEnd);
...
const PersonNetwork = (props) => {
  ...
  // props
  const { _id: propsId, relatedLinks, relatedNodes } = props;
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const response = await fetch(
        `${APIPath}item-network?_id=${propsId}&step=${step}`
      );
      ...
      const sData = await streamedResponse.text();
  ...
  useEffect(() => {
    ...
    const drawGraph = () => {
      ...
      d3.select('#graph-zoom-in').on('click', () => {
        zooHandler.scaleBy(canvas, 1.25);
      });
      d3.select('#graph-zoom-out').on('click', () => {
        zooHandler.scaleBy(canvas, 0.75);
      });
      d3.select('#graph-pan-up').on('click', () => {
        ...
        zooHandler.translateBy(canvas, newX, newY);
      });
      ...
      zooHandler(d3.select(ctx.canvas));
      updateDimensions();
    };
  ...
  const loadNodeDetails = async (_id) => {
    if (_id === propsId) {
      ...
      const responseData = await axios({
        ...
        crossDomain: true,
        params: { sourceId: propsId, targetId: _id, step },
      })
  ...
  const centerNode = (_id) => {
    ...
    if (typeof node !== 'undefined') {
      ...
      node.selected = true;
      zooHandler.translateTo(canvas, node.x, node.y);
    }
  };
...
PersonNetwork.propTypes = {
  ...
  relatedLinks: PropTypes.func,
  relatedNodes: PropTypes.func,
};
