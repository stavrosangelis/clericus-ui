...
const Block = props => {
  ...
  if (propsResources.length>0) {
    let resources = propsResources.filter(p=>p.ref.label.toLowerCase().includes(simpleSearchSet.toLowerCase()));
    let resourcesData = [];
    for (let i=0;i<resources.length; i++) {
      ...
      resourcesData.push(<li key={resource.ref._id}><Link className="tag-bg tag-item" href={url} to={url}><i>{resource.term.label}</i> {resource.ref.label}</Link></li>)
    }
    ...
    resourcesRow = <div key="resourcesRow">
      <h5>Resources <small>[{resources.length}]</small>
        <div className="btn btn-default btn-xs pull-right toggle-info-btn pull-icon-middle" onClick={(e)=>{props.toggleTable(e,"resources")}}>
